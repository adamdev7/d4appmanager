...
class AIEmailAssistantSettings(Base):
    ...
    # WhatsApp ping when mail is held for an admin (uses the shared user connection)
...
class AnalyticsManualInvestment(Base):
    """Manual cash investments that cannot be pulled from an API (ads tools, agencies, etc.)."""

    __tablename__ = "analytics_manual_investments"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=_uuid)
    store_id: Mapped[str] = mapped_column(String(36), ForeignKey("stores.id", ondelete="CASCADE"), index=True)
    label: Mapped[str] = mapped_column(String(128), default="Investment")
    amount: Mapped[str] = mapped_column(String(16), default="0")
    investment_date: Mapped[str] = mapped_column(String(10), index=True)  # YYYY-MM-DD
    note: Mapped[str | None] = mapped_column(String(255), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
...
class CreativeGenerationJob(Base):
    ...
    progress_step: Mapped[str] = mapped_column(String(32), default="")
    progress_pct: Mapped[int] = mapped_column(Integer, default=0)
    progress_log_json: Mapped[str] = mapped_column(Text, default="[]")
